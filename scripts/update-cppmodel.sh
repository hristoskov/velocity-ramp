...
case "$OS_NAME" in
    Linux)
        ...
        case "$ARCH" in
            x86_64|aarch64) ;;
            *) echo "Unsupported arch '$ARCH' for Linux. Set ARCH=x86_64|aarch64."; exit 1 ;;
        esac
    ...
    Darwin)
        ...
        case "$ARCH" in
            x86_64|arm64) ;;
            *) echo "Unsupported arch '$ARCH' for macOS. Set ARCH=x86_64|arm64."; exit 1 ;;
        esac

        PLATFORM_TAG="${OS_TAG}-${ARCH}"
        ;;
    *)
        echo "Unsupported OS '$OS_NAME'. This script supports Linux and macOS; use scripts/update-cppmodel.ps1 on Windows."
        exit 1
        ;;
esac
